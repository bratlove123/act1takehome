...
| `OCEANS_X_ACCESS_KEY` | `.env` | `secrets.OCEANS_X_ACCESS_KEY` (fallback test key if unset on forks) | Vercel env + GH secret — unlocks `/oceans-x` portal |
...
2. Create a Vercel project from this repo; set env vars `DATABASE_URL`, `EXTERNAL_API_KEY`, `OCEANS_X_API_KEY`, and `OCEANS_X_ACCESS_KEY` in the Vercel dashboard (Production), **or** let the Deploy workflow sync them from GitHub secrets.
...
5. Add GitHub Actions secrets:
   ...
   - `OCEANS_X_ACCESS_KEY` — portal unlock key for `/oceans-x` (server-side only)
