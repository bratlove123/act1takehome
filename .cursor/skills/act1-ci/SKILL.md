...
Needs secrets: `DATABASE_URL`, `EXTERNAL_API_KEY`, `OCEANS_X_API_KEY`, `OCEANS_X_ACCESS_KEY`, `VERCEL_TOKEN` (team/account scope), `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`.

Deploy job links the project via `.vercel/project.json` (from secrets), then `vercel deploy --prod --yes`. Set `OCEANS_X_ACCESS_KEY` in Vercel production env (portal unlock for `/oceans-x`).
